...
public partial class RevokeToken : ComponentBase
{
    [Inject] public DefaultsService DefaultsService { get; set; } = default!;

    private Network _network = default!;
    private RevokeTokenInput _input = new RevokeTokenInput();
    ...
    protected async Task HandleValidSubmit()
    {
        ...
        {
            _output = await client.RevokeTokenKycAsync(_input.Token!, _input.Account!, ctx => ctx.Memo = _input.Memo?.Trim());
        });
    }
